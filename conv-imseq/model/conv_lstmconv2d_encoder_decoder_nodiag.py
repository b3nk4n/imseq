...
def _conv_stack(x, reg_lambda, is_training, memory_device,
    ...
    x = tf.contrib.layers.batch_norm(x, is_training=is_training,
                                     updates_collections=updates_collections, scope="x_bn")
    ...
    conv1 = tf.contrib.layers.batch_norm(conv1, is_training=is_training,
                                         updates_collections=updates_collections, scope="conv1_bn")
    ...
    conv2 = tf.contrib.layers.batch_norm(conv2, is_training=is_training,
                                         updates_collections=updates_collections, scope="conv2_bn")
    ...
    conv3 = tf.contrib.layers.batch_norm(conv3, is_training=is_training,
                                         updates_collections=updates_collections, scope="conv3_bn")
    return conv3


def _deconv_stack(representation, reg_lambda, channels, is_training, memory_device,
                  updates_collections=tf.GraphKeys.UPDATE_OPS):
    representation = tf.contrib.layers.batch_norm(representation, is_training=is_training,
                                                  updates_collections=updates_collections, scope="rep_bn")
    ...
    deconv1 = tf.contrib.layers.batch_norm(deconv1, is_training=is_training,
                                           updates_collections=updates_collections, scope="deconv1_bn")
    ...
    deconv2 = tf.contrib.layers.batch_norm(deconv2, is_training=is_training,
                                           updates_collections=updates_collections, scope="deconv2_bn")
    deconv3 = tt.network.conv2d_transpose("Deconv3", deconv2, channels,
                                          ...
                                          device=memory_device)
                                          # Note: No-Non-lin. give best results for MovingMNIST when no BN is used!
    return deconv3
        ...
        with tf.variable_scope("ConvStack") as varscope:
            ...
            for i in xrange(len(input_seq)):
                if i > 0:
                    ...
                    uc = tf.GraphKeys.UPDATE_OPS  # to only add the BN update op once
                    
                c3 = _conv_stack(input_seq[i], self.reg_lambda,
                                 is_training, memory_device,
                                 updates_collections=uc)
        ...
        with tf.variable_scope("DeconvStack") as varscope:
            ...
            for i in xrange(len(rep_outputs)):
                ...
                dc3 = _deconv_stack(rep_outputs[i], self.reg_lambda, input_shape[4],
                                    is_training, memory_device, updates_collections=uc)
